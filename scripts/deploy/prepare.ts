...
import { RELEASE_ASSET_PREFIX, RELEASE_TAG_PATTERN, STORE_UPLOAD_DIRECTORY } from '../constants';
...
export const prepare = (env: NodeJS.ProcessEnv = process.env): void => {
    ...
    const mode = env.DEPLOY_MODE || 'submit';
    if (!['chrome', 'firefox'].includes(browser || '') || !['submit', 'status'].includes(mode)) {
    ...
    requireConfiguration(browser === 'firefox'
        ? ['FIREFOX_CLIENT_ID', 'FIREFOX_CLIENT_SECRET', 'FIREFOX_AMO_ID']
        : ['CHROME_APP_ID', 'CHROME_PUBLISHER_ID', 'CHROME_CLIENT_ID', 'CHROME_CLIENT_SECRET',
            'CHROME_REFRESH_TOKEN'], env);
    ...
    const assets = [archive, ...(browser === 'firefox' ? [source] : [])];
