...
import { GECKO_ID } from '../../scripts/constants';
...
const env = {
    ...
    VERSION: '1.2.3', GITHUB_OUTPUT: 'fixture-output', GITHUB_STEP_SUMMARY: 'fixture-summary',
};
const addon = { guid: GECKO_ID, slug: 'fixture', status: 'unreviewed', categories: ['appearance'] };
const pending = {
    id: 123, version: '1.2.3', channel: 'listed', source: 'https://example.test/source.zip',
...
beforeEach(() => {
    ...
    vi.mocked(readFileSync).mockReturnValue('Reproduce with pnpm install --frozen-lockfile; pnpm release firefox');
...
describe('Firefox deployment orchestration', () => {
    ...
    it('refuses new submission without reviewer notes', async () => {
        ...
        await expect(run({ ...env, AMO_OPERATION: 'preflight' })).rejects.toThrow('AMO_REVIEW.md');
    ...
    it('reports pending review immediately without polling or writing a signed artifact', async () => {
        ...
        expect(appendFileSync).toHaveBeenCalledWith('fixture-summary', expect.stringContaining('awaiting Mozilla'));
    });
    it('never treats a status outage as permission to upload', async () => {
        request.mockResolvedValueOnce(new Response(null, { status: 503 }));
        await expect(run({ ...env, AMO_OPERATION: 'preflight' })).rejects.toThrow('HTTP 503');
        expect(appendFileSync).not.toHaveBeenCalled();
    });
    it('rejects an unexpected listing identity before looking up or creating versions', async () => {
        ...
        await expect(run({ ...env, AMO_OPERATION: 'preflight' })).rejects.toThrow('Gecko ID mismatch');
