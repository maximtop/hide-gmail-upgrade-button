...
3. `.github/workflows/release.yml` rejects a tag that does not match
   ...
   - `hide-gmail-upgrade-button-<version>-chrome.zip`,
     `hide-gmail-upgrade-button-<version>-edge.zip`, and
     `hide-gmail-upgrade-button-<version>-firefox.zip`, the store-ready
     archives
   - `hide-gmail-upgrade-button-<version>-source.zip`, the tagged repository
...
Later versions go through the workflow.
...
Do not submit it again. The next release can use the workflow.

## Store configuration

The store items must exist before any deployment: no store API can create
the listing. One-time setup per store:
...
4. Run the store's deploy workflow once with `mode: validate`, then with
   `mode: submit`.
...
| `op run --env-file=.env.1password -- go-webext status edge -a "$EDGE_PRODUCT_ID"` | Print the Edge product state. |
